...
/**
  ...
  * @retval 小于0表示发送失败
  */
int ICACHE_FLASH_ATTR mqtt_publish(char *pTopic,char *pMessage)
{
  int32_t len,rc;
  ...
  transport_open();	
  os_printf("mqtt published:%s \r\n",buf);
  rc = transport_sendPacketBuffer(buf,len);
  transport_close();
	if (rc == len)
		os_printf("Successfully published\n\r");
	else
		os_printf("Publish failed\n\r");
  return 0;
}
/**
  ...
  * @retval 小于0表示订阅消息失败
  */
int ICACHE_FLASH_ATTR mqtt_subscrib(char *pTopic,char *pMessage)
{
	MQTTPacket_connectData data = MQTTPacket_connectData_initializer;
	int rc = 0;
	unsigned char buf[200];
	...
	rc = transport_open();
	if(rc < 0){
    os_printf("transport_open error\n\r");
		return rc;
  }
	...
	rc = transport_sendPacketBuffer(buf, len);
  if(rc != len){
    os_printf("connect transport_sendPacketBuffer error\n\r");
    goto exit;
  }
	/* wait for connack */
	if (MQTTPacket_read(buf, buflen, transport_getdata) == CONNACK)
	{
		...
		{
			os_printf("Unable to connect, return code %d\n\r", connack_rc);
			goto exit;
		}
	}else{
    os_printf("MQTTPacket_read error\n\r");
		goto exit;
  }
